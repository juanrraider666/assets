...
$('.sponsors_btn').click(function(){
    ...
    google.charts.setOnLoadCallback(drawChart_ogd);
   google.charts.setOnLoadCallback( drawChart_op_m); google.charts.setOnLoadCallback(drawRegionsMap);
...
jQuery(document).ready(function(){
 ...
 /*  $(".bootstrap-select.form-control").click(function(){
       $(this).find('.dropdown-menu.open').addClass("scrollbar-outer");
   $('.bootstrap-select .dropdown-menu.open').scrollbar({
		ignoreMobile : true,
	});
...
$('#modal_fs_spo').on('shown.bs.modal', function (e) {
    slider = $('.sponsors_bx').bxSlider({
	pager:false,
	adaptiveHeight:true,
...
$('#modal_fs').on('shown.bs.modal', function (e) {
   $('.swtch_txt, .swtch_txt2').circleType({fluid:true, });
	...
	winner_slide = $('.slide_winners').bxSlider({
	pager:false,
	adaptiveHeight:true,
...
$('#porcentage_overall').html(function() {
  ...
  return txt.substr(0, txt.length-3)          //all but the last two characters
         + '<span>'+txt.slice(-3)+'</span>';  //put last two charwrap_filter_scrollacters in a span
});


var $table = $('table.header_fix');
$table.floatThead({
	position: 'absolute',
    scrollContainer: function($table){
        return $table.closest('.table-responsive1');
    }
});
